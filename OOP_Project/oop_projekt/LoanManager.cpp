...
std::shared_ptr<Loan> LoanManager::addLoan(int keyID, std::shared_ptr<Member> member, std::shared_ptr<Librarian> librarian, const std::string& dateOfLoan)
{
    if (!member->canLoan()) {
        std::cerr << "ERROR: This member has reached the loan limit !!!" << std::endl;
        return nullptr;
    }
    this->loan_list.emplace(keyID, std::make_shared<Loan>(keyID, member, librarian, dateOfLoan));
...
{
    for (const auto& l : this->loan_list) {
