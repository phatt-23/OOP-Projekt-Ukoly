...
Client* Bank::get_client(int t_c) {
    ...
    if (DBG)std::cout << ERROR << "Client* Bank::get_client(int t_c)"
...
Account* Bank::get_account(int t_a) {
    ...
    if (DBG)std::cout << ERROR << "Account* Bank::get_account(int t_a)"
...
bool Bank::m_check_if_exits(int t_n) {
    for (int i = 0; i < m_accounts_index; ++i) {
        ...
        if (DBG)std::cout << ERROR << "bool Bank::m_check_if_exits(int t_n)"
...
Account* Bank::create_account(int t_n, Client* t_c) {
    if (t_c != nullptr && !m_check_if_exits(t_n)) {
        ...
        m_accounts_index++;
        if (DBG) std::cout << NOTE << "Account* Bank::create_account(int t_n, Client* t_c)"
    ...
    if (DBG) std::cout << ERROR << "Account* Bank::create_account(int t_n, Client* t_c)"
...
Account* Bank::create_account(int t_n, Client* t_c, double t_ir) {
    ...
    if (DBG)
        std::cout << ERROR << "Account* Bank::create_account(int t_n, Client* t_c, double t_ir)"
        << "Account wasn't created!"
        << std::endl;
    return nullptr;
}
...
void Bank::add_interest() {
    ...
    if (DBG)
        std::cout << NOTE << "void Bank::add_interest()"
...
bool Bank::deposit_for(Account* t_a, double t_d) {
    for (int i = 0; i < m_accounts_index; ++i) {
        if (m_accounts[i] == t_a) {
            ...
            if (DBG)
                std::cout << NOTE << "bool Bank::deposit_for(Account* t_a, double t_d)"
    ...
    std::cout << ERROR << "bool Bank::deposit_for(Account* t_a, double t_d)"
...
bool Bank::withdraw_for(Account* t_a, double t_d) {
    for (int i = 0; i < m_accounts_index; ++i) {
        if (m_accounts[i] == t_a) {
            ...
            if (m_accounts[i]->withdraw(t_d)) {
                if (DBG)
                    std::cout << NOTE << "bool Bank::withdraw_for(Account* t_a, double t_d)"
    ...
    std::cout << ERROR << "bool Bank::withdraw_for(Account* t_a, double t_d)"
...
PartnerAccount* Bank::create_account(int t_n, Client* t_c, Client* t_p)
{
    if (t_c != nullptr && t_p != nullptr && !m_check_if_exits(t_n)) {
        ...
        m_accounts_index++;
        if (DBG)
            std::cout << NOTE << "PartnerAccount* Bank::create_account(int t_n, Client* t_c, Client* t_p)"
            << "Accout for client ("
            << t_c->get_code() << ", " << t_c->get_name() << ")"
            << " alongside with partner ("
            << t_p->get_code() << ", " << t_c->get_name() << ")"
    ...
    if (DBG)
        std::cout << ERROR << "PartnerAccount* Bank::create_account(int t_n, Client* t_c, Client* t_p)"
        << "Account wasn't created!"
        << std::endl;
    return nullptr;
}
PartnerAccount* Bank::create_account(int t_n, Client* t_c, Client* t_p, double t_ir) {
    ...
    if (DBG)
        std::cout << ERROR << "PartnerAccount* Bank::create_account(int t_n, Client* t_c, Client* t_p, double t_ir"
        << "Account wasn't created!"
        << std::endl;
    return nullptr;
}
...
CreditAccount* Bank::create_credit_account(int t_n, Client* t_c, double credit) {
    ...
    if (DBG)
        std::cout << ERROR << "CreditAccount* Bank::create_credit_account(int t_n, Client* t_c, double credit)"
        << "Account wasn't created!"
        << std::endl;
    return nullptr;
}
