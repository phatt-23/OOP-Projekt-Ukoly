...
{
    if (DBG) printf("INFO: Account::Account(int t_n, Client* t_c)\n");
...
Account::Account(int t_n, Client* t_c, double t_ir)
    ...
    , m_owner(t_c)
{
    if (DBG)printf("INFO: Account::Account(int t_n, Client* t_c, double t_ir)\n");
    Account::s_account_count += 1;
}
...
Account::Account(int t_n, Client* t_c, Client* t_p)
    ...
    , m_partner(t_p)
{
    if (DBG)printf("INFO: Account::Account(int t_n, Client* t_c, Client* t_p)\n");
    Account::s_account_count += 1;
}
...
Account::Account(int t_n, Client* t_c, Client* t_p, double t_ir)
    ...
    , m_partner(t_p)
{
    if (DBG)printf("INFO: Account::Account(int t_n, Client* t_c, Client* t_p, double t_ir)\n");
    Account::s_account_count += 1;
}
...
bool Account::withdraw(double t_a) {
    if (can_withdraw(t_a)) {
        ...
        if (0) std::cout << ERROR << "bool Account::withdraw(double t_a)"
...
Account::~Account() {
    // if(DBG)
    printf("INFO: void Account::add_interest()\n");
}
